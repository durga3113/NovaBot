...
const novaInit = require('baileys').default;
...
const app = express();
const store = require('baileys').makeInMemoryStore({ logger: pino().child({ level: 'silent', stream: 'store' }) });
...
async function startNova() {
  try {
    const nova = novaInit({
      ...
      auth: (await require('baileys').useMultiFileAuthState("./clear.json")).state
    });
...
function ConnectionUpdate(update) {
  ...
  switch (Boom(lastDisconnect?.error)?.output.statusCode) {
    case require('baileys').DisconnectReason.connectionClosed:
      console.log("Connection closed, reconnecting...");
      startNova();
      break;
    case require('baileys').DisconnectReason.loggedOut:
      clearState();
      console.log("Device Logged Out, Please Delete Session and Scan Again.");
      process.exit();
      break;
    case require('baileys').DisconnectReason.restartRequired:
      console.log("Server starting...");
      startNova();
      break;
    case require('baileys').DisconnectReason.timedOut:
